...
function getToken() {
	...
	$row = null;
	if ($result = $botServer->getDB()->query($query)) {
		$row = $result->fetch_assoc();
		setLogMsg(3, __FILE__ . ' : ' . __LINE__ . ' -- result = ' . var_export($row, true));
		$result->close();
	} else
		setLogMsg(3, __FILE__ . ' : ' . __LINE__ . ' -- result error = ' . $botServer->getDB()->error);	
...
function setToken($token, $expires_in) {
	...
	$query = sprintf("insert into %ssetting_list (name, value, expired_date) values ('access_token', '%s', %u) on duplicate key update value='%s', expired_date=%u", 
